...
function minvehvalue() {
  ...
  if (currentvalue < 10000) $('#currentvalue').val(10000)
}
...
function calcFireTheft() {
  ...
  var age = $('#yearage').val()
  var ftPercent = 0.04
  if (age>=7) ftPercent = 0.08
  var charge = currentvalue * ftPercent > 1000 ? currentvalue * ftPercent : 1000
...
function estNCD() {
  ...
  var est5 = (est11 * (adjncd / 100)).toFixed(2)  
  $('#est5').val(`$${est5}`)
}
...
function estSub4() {
  var est11 = parseFloat($('#est11').val().substring(1))
  var est5 = parseFloat($('#est5').val().substring(1))
  var est13 = (est11 - est5).toFixed(2)  
  $('#est13').val(`$${est13}`)
}
...
function processAge() {
  if (
    ...
    var resp =
    '<div id="frameRes" class="alert alert-info">'
...
$('#next').click(function(){
  ...
  if (state === 3) { //Final Stage
    var message = '<h2>Commercial Vehicle > 3rd Party Fire & Theft</h2><br>Date Format (YYYY-MM-DD)<hr>'
